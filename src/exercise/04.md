...
# Learnings
1. As we know that functions make our life easier by code sharing and reusability we can also share JSX code same as functions but these are called components

2. We pass element needed to be created as React.createElement method's first parameter, we can alsp pass function as parameter to React.createElement e.g. :- 
...
3. As we can pass function as parameter to React.createElement method, we can make it into a component by making the first letter of the function name capital and passing it just like JSX tag.
When you pass a element with capital letter to React.createElement method, it corresponds to a varible within the scope
e.g. :-
...
<div className="container">
    <Message>Hello world</Message>  ===> React.createElement(Message, "Hello world")
    <Message>Goodbye world</Message> ===> React.createElement(Message, "Goodbye world")
</div>

4. We can perform validations on props coming to the component through propTypes. prop-types is a package through which we can assign type to the each parameter and if it fails we can throw error.

5. React Fragment allows you to remove unneccessary node in the DOM tree. It will place two elements side by side(sibling nodes). <React.Fragment> or <>
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=devateja58%40gmail.com).
